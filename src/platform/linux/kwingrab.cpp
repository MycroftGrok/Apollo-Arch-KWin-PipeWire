...
static std::string apollo_local_get_env(const char *name) {
  const char *value = std::getenv(name);
  return value ? std::string {value} : std::string {};
}

static bool apollo_local_get_env(const char *name, std::string &out) {
  out = apollo_local_get_env(name);
  return !out.empty();
}
...
namespace kwin {
  ...
  public:
    ...
    static bool is_permission_system_deactivated() {
      return apollo_local_get_env("KWIN_WAYLAND_NO_PERMISSION_CHECKS") == "1";
    }
    ...
    static void setup() {
      ...
      auto filenameprefix = std::format("{}.kwin", PROJECT_NAME);
      ...
      if (create_file) {
        ...
        if (filestream.is_open()) {
          filestream << "[Desktop Entry]" << std::endl
                     ...
                     << "Name="sv << PROJECT_NAME << "-kwin-wayland-permission" << std::endl
  ...
  private:
    ...
    static std::filesystem::path get_home_dir() {
      ...
      if (std::string homedir = apollo_local_get_env("HOME"); !homedir.empty()) {
        return homedir;
      }
    ...
    static std::filesystem::path get_xdg_user_applications_path() {
      ...
      if (std::string dir = apollo_local_get_env("XDG_DATA_HOME"); !dir.empty()) {
    ...
    static bool check_kwin_system_permissions(const std::string_view &filenameprefix, const std::string_view &executablepath) {
      ...
      if (const std::string e = apollo_local_get_env("XDG_DATA_DIRS"); !e.empty()) {
  ...
  public:
    ...
    int init(const bool setup_permissions = true) {
      ...
      std::string wl_name;
      if (!apollo_local_get_env("WAYLAND_DISPLAY", wl_name)) {
