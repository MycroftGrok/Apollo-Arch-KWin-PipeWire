...
<template>
  <div class="mb-3">
    <label for="fallback_mode" class="form-label">Streaming Display Resolution</label>

    <select id="fallback_mode" class="form-select" v-model="config.fallback_mode">
      <option
        v-for="mode in resolutionOptions"
        :key="mode.value"
        :value="mode.value"
      >
        {{ mode.label }}
    ...
    <div class="form-text">
      Choose the display mode Apollo should use for streaming. Auto lets the client request the resolution.
    </div>
  </div>
